...
-- ── Цитирование фрагмента сообщения ─────────────────────────────────────────
-- Цитата — это обычный ответ (reply_to_id указывает на исходное сообщение),
-- у которого подменён текст превью: вместо начала всего сообщения показывается
-- ровно тот кусок, который выделил отправитель.
--
-- Поэтому отдельной таблицы нет и рендерится цитата тем же кодом, что и ответ.
-- Хранится именно текст, а не смещения: исходное сообщение можно отредактировать
-- или удалить, и тогда любые offset'ы указывали бы в пустоту, а цитата обязана
-- остаться ровно такой, какой её отправили.
ALTER TABLE messenger.messages ADD COLUMN IF NOT EXISTS quote_text text;

-- ── Альбомы фото/видео (медиагруппы) ────────────────────────────────────────
-- Отправленные одной пачкой фото и видео на клиенте показываются единым
-- компактным пузырём (как медиагруппа в Telegram), но в БД остаются, как и
-- раньше, отдельными строками — по одной на файл, каждая со своим id и своим
-- блобом. Их связывает media_group_id: одинаковое значение = один альбом.
--
-- Почему отдельной таблицы нет и почему это правильно именно для нашей модели
-- «одно вложение = одна строка»:
--   • /image/<id>, /video/<id> и просмотрщик альбома работают по id сообщения
--     как и прежде — ничего переписывать не нужно;
--   • группировка — чисто клиентская отрисовка: подряд идущие сообщения одного
--     автора с одинаковым непустым media_group_id рисуются одним пузырём;
--   • порядок внутри альбома = порядок id (файлы грузятся строго по очереди,
--     слева направо), поэтому отдельная колонка-индекс не нужна.
--
-- NULL — одиночное вложение вне альбома (в т.ч. все старые сообщения и любой
-- файл-невидео/нефото: такие в группы не входят — им нечего листать в
-- просмотрщике). Тип text: id генерирует клиент (crypto.randomUUID()), сервер
-- его не толкует, только хранит и отдаёт.
ALTER TABLE messenger.messages ADD COLUMN IF NOT EXISTS media_group_id text;

-- Индекс частичный (только непустые группы): единственный сценарий выборки по
-- этой колонке — «собрать альбом целиком», одиночные сообщения по ней не ищут,
-- поэтому NULL'ы в индекс не тащим.
